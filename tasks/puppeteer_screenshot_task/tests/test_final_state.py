...
def test_puppeteer_screenshot_task(start_trigger_dev):
    ...
    for _ in range(20): # Increased attempts for Puppeteer task which might take longer
        ...
        if run_status == "COMPLETED":
            ...
            path_key = next((k for k in output.keys() if 'path' in k.lower()), None)
            assert path_key is not None, f"No key containing 'path' found in output: {output}"
            screenshot_path = output[path_key]
